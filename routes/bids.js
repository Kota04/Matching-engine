...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    console.log(err);
    await releaseLock(key);  // Ensure lock release in case of error
...
const acquireLock = async (key) => {
  ...
  return await setAsync(key, 'Locked');
}
...
const matchOrder = async (currNode, memQueue) => {
  while (_.size(memQueue) > 0 && (_.get(currNode, 'trade_type') === false ? _.get(memQueue, '[0].price_per_unit') <= _.get(currNode, 'price_per_unit') : _.get(memQueue, '[0].price_per_unit') >= _.get(currNode, 'price_per_unit'))) {
    if (_.get(currNode, 'units') >= _.get(memQueue, '[0].units')) {
      ...
      await updateBidStatus(_.get(memQueue, '[0].id'), true, _.get(memQueue, '[0].units'));
      memQueue.shift();
    } else {
      _.update(memQueue, '[0].units', units => units - _.get(currNode, 'units'));
      console.log('hit');
      await insertTrade(memQueue[0], currNode, _.get(currNode, 'units'));
      await updateBidStatus(_.get(memQueue, '[0].id'), false, _.get(currNode, 'units'));
      return 0;
    }
  }
...
const insertTrade = async (currNode, inMemoryNode, units) => {
  ...
  const values = _.get(currNode, 'trade_type') === false
    ? [_.get(inMemoryNode, 'id'), _.get(currNode, 'user_id'), _.get(inMemoryNode, 'user_id'), _.get(currNode, 'channel_id'), units, _.get(inMemoryNode, 'price_per_unit')]
    : [_.get(inMemoryNode, 'id'), _.get(inMemoryNode, 'user_id'), _.get(currNode, 'user_id'), _.get(currNode, 'channel_id'), units, _.get(currNode, 'price_per_unit')];
  return await db.query(query, values);
}
